...
# text
ID_CORPUS, CORPUS = pickle.load(open('corpus', 'rb'))
VECTOR_DOC = vectorizer.fit_transform(CORPUS)

# content
FILENAME_LIST, IMG_CORPUS = pickle.load(open('corpus_img', 'rb'))
...
async def IR_contentbased(request: Request, file: UploadFile = File(...)):
    dir_query = 'static/img_query'
    for f in os.listdir(dir_query):
        os.remove(os.path.join(dir_query, f))
    if file:
        query_location=os.path.join(dir_query, file.filename)
